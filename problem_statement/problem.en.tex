...
On his first day, Billy discovers that the plant layout is unusual. Some buildings are connected by bidirectional footpaths, forming clusters called sectors. Within a sector, he can reach any building on foot. To move between sectors he must drive, and every sector is reachable from any other sector by road.
...
\caption{An example layout of the plant. There are three sectors, each connected by roads that Billy can drive on. Within each sector, all buildings are reachable on foot via bidirectional footpaths. For example, buildings 1 and 3 aren't directly connected but can be reached by walking through building 4.}
...
First line: Two integers separated by a space. The first integer gives the number of buildings (0 $<$ buildings $<$ 1000). The second integer gives the number of buildings to inspect (0 $<$ n $<$ 1000).

Second line: A sequence of n integers representing the id numbers of buildings to inspect.

The following lines: One line for each building (in no particular order), consisting of a sequence of integers. First, an integer giving the id number of the building. Second, the number of (other) buildings that can be reached from this building via bidirectional footpaths. Third, a sequence of distinct building id numbers indicating which buildings are directly connected to this building by footpaths. 

\textbf{Note:} Footpaths are bidirectional: if building A lists building B as a connection, then building B must also list building A. Also, every building is reachable from another within a sector.
